            ...
            <tr class="odd:bg-[#f8f8f8] even:bg-[#f2f2f2] hover:bg-[#f1f1f1]">
                ...
                @foreach ($columns as $col)
                    <td class="px-4 py-2 text-center">
                        ...
                        @elseif ($col === 'status' && $value === 'Pending')
                            <span class="bg-[#ffdd44] px-2 py-1 rounded-xl">
                                {{ $value }}
                            </span>
                        @elseif ($col === 'status' && $value === 'Valid')
                            <span class="bg-[#44c96a] px-2 py-1 rounded-xl text-[#fbfbfb]">
                                {{ $value }}
                            </span>
                        @elseif ($col === 'status' && $value === 'Rejected')
                            <span class="bg-[#e73424] px-2 py-1 rounded-xl text-white">
                                {{ $value }}
                            </span>
                        @else
                            {{ $value ?? '-' }}
                ...
                @if ($status === 'Draft')
                    ...
                    <td class="px-4 py-2">
                        <div class="flex justify-center items-center gap-3">
                            <button type="button"
                                class="flex justify-center items-center gap-2 cursor-pointer px-3 py-0.5 text-white bg-[#2179ca] hover:bg-[#1c6bb4] rounded-sm"
                                if @click="$dispatch('{{ $editEvent }}', {{ json_encode($row) }})">
                                <img src="/icon/edit.svg" alt="edit-icon" class="w-[15px] h-[15px]"> Edit
                            </button>

                            <button type="button"
                                class="flex justify-center items-center gap-2 cursor-pointer px-3 py-0.5 bg-red-500 hover:bg-red-600 text-white rounded-sm"
